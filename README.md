...
## Day 11-6
首先还是使用她的dataset和detaloader，但是不知道会不会出现什么不会的东西在里面导致程序跑不通，所以明天需要做的第一件事就是要首先使用数据集测试一下提供的dataset和dataloader的迭代输出是什么样的， 然后完成相应的模型forward函数，，然后就是模型初始化需要的参数确定要怎样传入
## Day 11-4
